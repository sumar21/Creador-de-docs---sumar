...
export function PublicDocPage({ slug, data, printMode = false }: PublicDocPageProps) {
  const endAction = !printMode ? (
    <a href={`/api/pdf/${slug}`} className="sumar-button-primary inline-flex items-center gap-2">
  ...
  return (
    <main className={`${printMode ? "min-h-dvh" : "h-dvh overflow-hidden"} bg-app px-4 py-4 text-zinc-100 md:px-8`}>
      <div className={`mx-auto max-w-[1920px] ${printMode ? "" : "flex h-full min-h-0 flex-col"}`}>
        <div className={printMode ? "" : "min-h-0 flex-1"}>
          <ProposalDocument
            data={data}
            printMode={printMode}
